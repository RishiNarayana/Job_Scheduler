...
const Queue_js_1 = require("../../api/src/models/Queue.js");
const Job_js_1 = require("../../api/src/models/Job.js");
const JobExecution_js_1 = require("../../api/src/models/JobExecution.js");
const JobLog_js_1 = require("../../api/src/models/JobLog.js");
const DeadLetterQueue_js_1 = require("../../api/src/models/DeadLetterQueue.js");
const RetryPolicy_js_1 = require("../../api/src/models/RetryPolicy.js");
const retry_js_1 = require("../../api/src/utils/retry.js");
class JobProcessor {
    workerId;
    running = false;
    ...
    async processNextJob() {
        ...
        const queues = await Queue_js_1.Queue.find({ isPaused: false }).populate('retryPolicyId');
        if (queues.length === 0)
            return false;
        ...
        for (const queue of queues) {
            ...
            if (queue.concurrencyLimit !== undefined && queue.concurrencyLimit !== null) {
                const activeJobsCount = await Job_js_1.Job.countDocuments({
                    queueId: queue._id,
            ...
            const now = new Date();
            const job = await Job_js_1.Job.findOneAndUpdate({
                queueId: queue._id,
    ...
    async runJob(job, queue) {
        ...
        const execution = await JobExecution_js_1.JobExecution.create({
            jobId: job._id,
        ...
        const logInfo = async (msg) => {
            ...
            await JobLog_js_1.JobLog.create({ jobExecutionId: execId, level: 'info', message: msg, timestamp: new Date() });
        };
        const logWarn = async (msg) => {
            console.warn(`[Job ${job._id}] WARN: ${msg}`);
            await JobLog_js_1.JobLog.create({ jobExecutionId: execId, level: 'warn', message: msg, timestamp: new Date() });
        };
        const logError = async (msg) => {
            console.error(`[Job ${job._id}] ERROR: ${msg}`);
            await JobLog_js_1.JobLog.create({ jobExecutionId: execId, level: 'error', message: msg, timestamp: new Date() });
        };
        ...
        try {
            ...
            if (job.attemptsMade < job.maxAttempts) {
                ...
                if (policyId) {
                    const policy = await RetryPolicy_js_1.RetryPolicy.findById(policyId);
                    if (policy) {
                        delayMs = (0, retry_js_1.calculateRetryDelay)({
                            type: policy.type,
                ...
                await DeadLetterQueue_js_1.DeadLetterQueue.create({
